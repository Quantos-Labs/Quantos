#!/usr/bin/env python3
"""Deploy VybssIDO to Quantos testnet.

Usage:
    QTEST_ADDRESS=QTS:... SQTEST_ADDRESS=QTS:... python3 scripts/deploy_ido.py
"""
import json
import os
import subprocess
...
def main():
    ido_dir = Path(__file__).parent.parent / "solidity-contracts" / "ido"

    wasm = ido_dir / "VybssIDO.wasm"
    contract_meta = ido_dir / "VybssIDO.contract"
    if not wasm.exists() or not contract_meta.exists():
        fail(f"Missing {wasm} or {contract_meta}. Compile first:\n"
             f"  solang compile {ido_dir / 'VybssIDO.sol'} --target polkadot --output {ido_dir}")
    ...
    # Deploy VybssIDO(qtest, sqtest)
    print("\n1. Deploying VybssIDO...")
    ...
    ido_addr = resp.get("contract_address") or resp.get("address")
    if not ido_addr:
        fail(f"Deploy failed: {resp}")
    print(f"   VybssIDO: {ido_addr}")
    ...
    out_file = ido_dir / "deployed_ido_address.txt"
    with open(out_file, "w") as f:
        f.write(f"DEPLOYER={deployer}\n")
        f.write(f"IDO={ido_addr}\n")
