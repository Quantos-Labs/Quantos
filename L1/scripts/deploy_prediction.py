#!/usr/bin/env python3
"""Deploy VybssPrediction to Quantos testnet.

Usage:
    QTEST_ADDRESS=QTS:... SQTEST_ADDRESS=QTS:... python3 scripts/deploy_prediction.py
"""
import json
import os
import subprocess
...
def main():
    pred_dir = Path(__file__).parent.parent / "solidity-contracts" / "predictions"

    wasm = pred_dir / "VybssPrediction.wasm"
    contract_meta = pred_dir / "VybssPrediction.contract"
    if not wasm.exists() or not contract_meta.exists():
        fail(f"Missing {wasm} or {contract_meta}. Compile first:\n"
             f"  solang compile {pred_dir / 'VybssPrediction.sol'} --target polkadot --output {pred_dir}")
    ...
    print("=== VybssPrediction Deployment ===")
    ...
    # Deploy VybssPrediction(qtest, sqtest)
    print("\n1. Deploying VybssPrediction...")
    ...
    pred_addr = resp.get("contract_address") or resp.get("address")
    if not pred_addr:
        fail(f"Deploy failed: {resp}")
    print(f"   VybssPrediction: {pred_addr}")

    # Write address
    out_file = pred_dir / "deployed_prediction_address.txt"
    with open(out_file, "w") as f:
        f.write(f"DEPLOYER={deployer}\n")
        f.write(f"PREDICTION={pred_addr}\n")
