// PQC-Guard Sui — verification tests (no chain objects needed).
//
...
module quantos::pqc_guard_tests {
    ...
    fun test_quorum_reached() {
        let digest = hash::keccak256(&b"authorize this");
        let seed0 = word(1);
        let seed1 = word(2);
        let id0 = word(0x11);
        let id1 = word(0x22);
        ...
        let mut sp0 = vector::empty<vector<u8>>();
        vector::push_back(&mut sp0, leaf1); // sibling for index 0
        let mut sp1 = vector::empty<vector<u8>>();
        vector::push_back(&mut sp1, leaf0); // sibling for index 1

        let mut blob = vector::empty<u8>();
        push_u32(&mut blob, 2);
    ...
    fun test_not_in_set_rejected() {
        ...
        // attestor 0 valid; second uses a fabricated id NOT in the set.
        ...
        push_u32(&mut blob, 2);
        encode_proof(&mut blob, &id0, &root0, 0, &sig0, &empty, 0, &sp0);
        // fake_id ⇒ attestor_leaf differs ⇒ set membership fails.
        encode_proof(&mut blob, &fake_id, &root1, 0, &sig1, &empty, 1, &sp1);

        // Only 1 finalized member ⇒ below threshold 2.
